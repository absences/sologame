...
namespace SudokuBase
{
    public class SudokuMarket
    {
        ...
        /// 坐标与单元格的信息
        /// </summary>
        private Dictionary<(int,int), CellInfo> cellInfos = new Dictionary<(int, int), CellInfo>();


        public Dictionary<(int, int), CellInfo> GetCellInfos()
        {
            return cellInfos;
        }
        ...
        private List<(int, int)> GetRestLoctions(int restValue)
        {
            return cellInfos.Where(c => c.Value.GetRest().Contains(restValue)).Select(c => c.Key).ToList();
        }


        private Dictionary<int, List<(int, int)>> restLoction = new Dictionary<int, List<(int, int)>>();
